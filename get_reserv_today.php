...
if(isset($_POST['data']) == 1)
{
 include_once("include/Conn.php");
    ...
    echo "</h2>";
        ...
        $sql_today .= " JOIN room as r on(r.id_room = rs.id_room)";
        $sql_today .= " WHERE rs.startday='".$today."' ";
        $query_today = mysql_query($sql_today);
        $num_today = mysql_num_rows($query_today);
        if($num_today == 0)
        {
            echo "<center><h1><i class='ace-icon fa fa-exclamation-triangle'></i>ไม่มีรายการจองในวันนี้</h1></center>";
        }else
        {
            while($row_today = mysql_fetch_array($query_today))
            {
                echo "<h1>";
                echo "<i class='fa fa-star' aria-hidden='true'></i>&nbsp;".$row_today['name_room'];
                echo "</h1>";
